        document.addEventListener('DOMContentLoaded', () => {
            ...
            const MD_PARTS = [];
            // __MD_PARTS_START__
            MD_PARTS.push(String.raw`# 第三章：须弥·虚空劫灰**往世书**
...
`);
           ...
           const md = MD_PARTS.join('\n');
            ...
            function renderTable(blockLines, footnoteMap) {
                ...
                // 识别特殊表格
                const isLeTuWaLiTable = headerCells.length >= 2 &&
                    headerCells[0].includes('人物') &&
                    headerCells[1].includes('介绍');

                const isJiaoLingYuanTable = headerCells.length >= 3 &&
                    headerCells[0].includes('职称') &&
                    headerCells[1].includes('介绍') &&
                    headerCells[2].includes('主要人物');

                // 竖向合并：第 1 列空白视作沿用上一行
                // 教令院表格：不启用任何合并
                const rowspan = isJiaoLingYuanTable ? null : new Array(bodyCells.length).fill(1);
                const skipFirstCol = isJiaoLingYuanTable ? null : new Array(bodyCells.length).fill(false);
                let lastNonEmptyRow = -1;
                if (!isJiaoLingYuanTable) {
                ...
                for (let r = 0; r < mergedBody.length; r++) {
                    ...
                    for (let c = 0; c < colCount; c++) {
                        if (skipFirstCol && c === 0 && skipFirstCol[r]) continue;
                        ...
                        if (rowspan && rowspan[r] && c === 0 && rowspan[r] > 1) td.rowSpan = rowspan[r];
            ...
            function parseMarkdownToPage(markdown) {
                ...
                function renderImage(imgLine) {
                    ...
                    img.src = `../../img/context/Sumeru/${file}`;
                    img.alt = file;
